...
function getCriticalFromAtRule(args) {
  ...
  options.css.walkAtRules('critical', function (atRule) {
    ...
    if (!atRule.nodes) {
      rule = atRule.root();
      rule.walkAtRules('critical', function (criticalRule) {
        return criticalRule.remove();
      });
    }
  ...
  //   if (postcss.root(acc[key]).toString().length > 0) {
  //     acc[key].walkAtRules('critical', atRule => {
  ...
  //   //     node => node.type !== 'atrule' && node.name !== 'critical'
  //   //   )
  //   // } else {
  //   //   acc.nodes = result[key]
  //   // }
  //   // return acc
  // }, {})
}