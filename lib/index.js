...
function buildCritical(options) {
  ...
  return function (css) {
    ...
    (0, _getCriticalRules.getCriticalRules)(css, preserve, outputDest).catch(function (err) {
      console.error(_chalk.red.bold('Error:') + ' ' + err);
      process.exit(1);
    }).then(function (criticalOutput) {
      return Object.keys(criticalOutput).reduce(function (init, cur) {
        ...
        (0, _postcss2.default)(minify ? [_cssnano2.default] : []).process(criticalCSS).then(dryRunOrWriteFile.bind(null, dryRun, filePath));
