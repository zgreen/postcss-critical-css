...
function correctSourceOrder(root) {
  const sortedRoot = _postcss.default.root();
...
function getCriticalRules(css, defaultDest) {
  const critical = (0, _atRule.getCriticalFromAtRule)({
    css,
    defaultDest
  });
  css.walkDecls('critical-selector', decl => {
    ...
    const childRules = value === 'scope' ? (0, _getChildRules.getChildRules)(css, parent) : []; // Sanity check, make sure we've got a root node
